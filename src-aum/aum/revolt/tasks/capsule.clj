...
(defn invoke
  [ctx {:keys [exclude-paths extra-paths capsule-type output-jar aliases name package version] :as config} target]
    ...
    (if-let [application (and artifact-id group-id (str group-id ":" artifact-id))]
      (utils/timed
       ...
       (let [deps-edn  (io/file "deps.edn")
             ...
             combined-aliases (tools.deps/combine-aliases deps-map [:git-deps])
             resolved-deps (tools.deps/resolve-deps deps-map combined-aliases)
